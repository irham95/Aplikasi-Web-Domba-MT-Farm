...
Class Mutasi extends CI_Controller{
	...
	public function penerimaan(){
		...
		$data['title'] = "Rekap Stok Data Domba";
		$data['menu'] = 3;
		$data['curr'] = $curr;
	...
	public function view($idmaster, $iddivisi=0){
		...
		if($row){
			$data['title'] = "Detail Stok Data : $row[nama]";
		}
	...
	public function add($idmaster){
		...
		$row = $this->cms->get_page("cc_master",$idmaster);
		$data['title'] = "Tambah Data Terima ".$row['nama'];
		$data['tgl'] = date("Y-m-d");
	...
	public function addproses($idmaster){
		...
		if(empty($post['cc_tgl']) or empty($post['cc_jml'])){
			...
			create_alert("success","Berhasil menyimpan data pemesanan","mutasi/add/$idmaster");
		}
	}
	...
	public function kirim($idmaster,$iddiv=0){
		...
		$row = $this->cms->get_page("cc_master",$idmaster);
		$data['title'] = "Tambah Data Kirim ".$row['nama'];
		$data['tgl'] = date("Y-m-d");
	...
	public function kirimproses($idmaster){
		...
		if(empty($post['cc_tgl']) or empty($post['cc_divisi']) or empty($post['cc_jml'])){
			...
			if($post['cc_jml'] > $stokmaster){
				post_session($post, $list_post);
				create_alert("error","Stok data tidak mencukupi untuk melakukan pengiriman dengan jumlah sekian. (Stok master : $stokmaster, dikirim : $post[cc_jml])","mutasi/kirim/$idmaster/$post[cc_divisi]");
			}
	...
	public function pengiriman(){
		$curr = shortcode_login();
		$data['title'] = "Rekap Stok Data per Divisi";
		$data['menu'] = 4;
		$data['curr'] = $curr;
	...
	public function detaildivisi($iddivisi){
		...
		$data['title'] = "Detail Rekap Stok di Divisi ".$list[$iddivisi];
		$data['curr'] = $curr;
	...
	public function terjual($idmaster, $iddivisi){
		...
		$data['title'] = "Stok \"$row[nama]\" Terpakai di Divisi ".$list[$iddivisi];
		$data['curr'] = $curr;
	...
	public function addterjual(){
		...
		if($post['cc_jml'] > $post['cc_terjual']){
			...
			if(empty($post['cc_jml']) or empty($post['cc_tgl'])){
				...
				create_alert("success","Berhasil menyimpan data keluar di divisi","mutasi/terjual/$post[idmaster]/$post[iddivisi]");
			}
		}
	}
